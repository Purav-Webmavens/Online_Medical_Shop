...
<%
	...
	if(bank.equals("SBI"))
	{
		...
		while(rs.next())
		{
			if(name.equals(rs.getString("Name")) && account_number.equals(rs.getString("Account_Number")) && pass_code.equals(rs.getString("Pass_Code")))
			{
				if(Integer.parseInt(amount)<=(Integer.parseInt(rs.getString("Amount"))))
				{
					...
					//out.print("<h4 style=\"color:green;text-align:left;margin-top:-975px;margin-left:270px;\">Amount Transfered</h4>");
					rd.forward(request, response);
				}
				else
				{
					out.print("<h4 style=\"color:red;text-align:left;margin-top:-975px;margin-left:270px;\">Insufficient Amount</h4>");
		...
		if(count==0)
		{
			out.print("<h4 style=\"color:red;text-align:left;margin-top:-975px;margin-left:270px;\">Wrong Credentials</h4>");
			RequestDispatcher rd=request.getRequestDispatcher("wallet.jsp");
			rd.forward(request,response);
		}
	}
